...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, pretrained=False):
        ...
        self.conv_for_P5 = Conv(transition_channels * 16, transition_channels * 4)

        self.conv_for_P3 = Conv(transition_channels * 16, transition_channels * 4)
        self.down_sample1 = Transition_Block(transition_channels * 4, transition_channels * 2)

        self.conv_for_P4 = Conv(transition_channels * 32, transition_channels * 16)
    ...
    def forward(self, x):
        ...
        # print(feat1.shape)  # 512, 80, 80
        # print(feat2.shape)  # 1024, 40, 40
        # print(feat3.shape)  # 1024, 20, 20

        P5 = self.sppcspc(feat3)
        # print(P5.shape)  # 512, 20, 20
        P5_conv = self.conv_for_P5(P5)
        # print(P5_conv.shape)  # 128, 20, 20
        P5_upsample = self.upsample(P5_conv)
        # print(P5_upsample.shape)  # 128, 40, 40

        P3 = feat1
        # print(P3.shape) # 512, 80, 80
        P3_conv = self.conv_for_P3(P3)
        # print(P3_conv.shape)  # 128, 80, 80
        P3_downsample = self.down_sample1(P3_conv)
        # print(P3_downsample.shape)  # 128, 40, 40

        P4 = feat2
        # print(P4.shape) # 1024, 40, 40
        P4 = self.conv_for_P4(P4)
        # print(P4.shape)  # 512, 40, 40
        # ----------------------------融合--------------------------------- #
        x_w = torch.mean(P4, dim=3, keepdim=True)
        x_w_h = torch.mean(x_w, dim=2, keepdim=True)
        # print(x_w_h.shape) # 512, 1, 1
        x_h = torch.mean(P3_downsample, dim=2, keepdim=True)
        x_h_c = torch.mean(x_h, dim=1, keepdim=True)
        # print(x_h_c.shape) # 1, 1, 40
        x_c = torch.mean(P5_upsample, dim=1, keepdim=True)
        x_c_w = torch.mean(x_c, dim=3, keepdim=True)
        # print(x_c_w.shape) # 1, 40, 1
        s_c = self.sigmoid(x_w_h)
        # print(s_c.shape) # 512, 1, 1
        x_h_c = s_c * x_h_c
        # print(x_h_c.shape) # 512, 1, 40
        x_c_w = s_c * x_c_w
        # print(x_c_w.shape)  # 512, 40, 1
        x_h_c_conv_relu = self.relu(self.bn(x_h_c))
        # print(x_h_c_conv_relu.shape) # 512, 1, 40
        x_c_w_conv_relu = self.relu(self.bn(x_c_w))
        # print(x_c_w_conv_relu.shape) # 512, 40, 1
        s_w = self.sigmoid(x_h_c_conv_relu)
        # print(s_w.shape) # 512, 1, 40
        s_h = self.sigmoid(x_c_w_conv_relu)
        # print(s_h.shape) # 512, 40, 1
        P4 = P4 * s_h.expand_as(P4) * s_w.expand_as(P4)
        # print(P4.shape)  # 512, 40, 40
